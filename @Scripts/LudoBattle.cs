...
public enum BattleState3 { PLAYERTURN, ENEMYTURN, WON, LOST }
public class LudoBattle : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Instantiate(Roro, SpawnPos, Quaternion.identity);
        monsterName = Roro.name;  // Roro의 프리팹 이름

        // 초기 텍스트 설정
        EncounterText.text = $"Ludo가 배틀을 신청해왔다!";
