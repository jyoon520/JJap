...
public enum BattleState2 { PLAYERTURN, ENEMYTURN, WON, LOST }
public class LunaBattle : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 초기 텍스트 설정
        EncounterText.text = $"Luna가 배틀을 신청해왔다!";
        ...
        AttackButton3.onClick.AddListener(OnLeftHealButtonClick);

        EnemyHPState();
    }
    private void Update()
    {
    playerState.PlayerHPState(); // hp바가 초기화 안됨. 왜 안되는지 모름.
    }
    ...
    {
        if (state != BattleState2.PLAYERTURN) return;  // 플레이어 턴이 아닐 때 공격하지 않음
